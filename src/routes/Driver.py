...
def driver():
    try:
        ...
        if ride == None:
            return jsonify({'error': 'ride id not found or does not exist'}), 500
        
        driver = DriverModel.get_Driver(ride["driver"])

        # Check if driver exists
        if driver == None:
            return jsonify({'error': 'driver id not found or does not exist'}), 500
        ...
        try:
            ...
            payment = PaymentModel.make_Payment(payment_information)

            # Update driver status and ride information
